...
import { UniformTableType, uniformTable } from './MockData';
import './style.css';

type UniformTableRow = UniformTableType & {
  key: string;
};

const UniformTable: React.FC = () => {
  const [form] = Form.useForm();
  const [data, setData] = useState<UniformTableRow[]>(
    uniformTable.map(item => ({ ...item, key: item.employeeId }))
  );
  const [editingKey, setEditingKey] = useState<string>('');
  const [editingDisbursementKey, setEditingDisbursementKey] =
    useState<string>('');
  const [editingDeductionKey, setEditingDeductionKey] = useState<string>('');
  ...
  const isEditing = (record: UniformTableRow) => record.key === editingKey;

  const isNewRow = (record: UniformTableRow) =>
    isEditing(record) && !record.employeeId;

  const getDisbursementColumns = (
    cancel: (record: UniformTableRow) => void
  ): TableColumnsType<UniformTableRow> => [
    {
      title: 'ลำดับ',
      dataIndex: 'index',
      width: 60,
      render: (_, _record: UniformTableRow, index: number) => (
        <span>{index + 1}</span>
      ),
    },
    {
      ...
      width: 88,
      sorter: (a: UniformTableRow, b: UniformTableRow) => {
      ...
      render: (text: string, record: UniformTableRow) => {
        if (isNewRow(record)) {
          return {
            children: (
              <Form.Item
    ...
    {
      title: 'เบิกชุดฟอร์ม',
      dataIndex: 'uniformDisbursement',
      width: 116,
      render: (value: number, record: UniformTableRow) => {
        if (isNewRow(record)) {
          return (
            <Form.Item
              name="uniformDisbursement"
              className="m-0 mt-[-4.5px] mb-[-5.5px]"
              rules={[
                {
                  validator: (_, value) => {
        ...
        } else if (record.key === editingDisbursementKey) {
          return (
            <Form.Item
              name={`uniformDisbursement-${record.key}`}
              className="m-0 mt-[-4.5px] mb-[-5.5px]"
              rules={[
                {
                  validator: (_, value) => {
            ...
            >
              <Input
                ...
                autoComplete="off"
                autoFocus
                onPressEnter={() => saveDisbursement(record)}
                onKeyDown={e => {
                  if (e.key === 'Escape') {
                    setEditingDisbursementKey('');
                    form.resetFields([`uniformDisbursement-${record.key}`]);
                  }
                }}
              />
            </Form.Item>
        ...
        return (
          <div
            ...
            onClick={() => {
              setEditingDisbursementKey(record.key);
              form.setFieldsValue({
                [`uniformDisbursement-${record.key}`]:
                  record.uniformDisbursement,
              });
            }}
          >
            <span
    ...
    },
    {
      ...
      render: (value: number, record: UniformTableRow) => {
        ...
        } else if (record.key === editingDeductionKey) {
          return (
            <Form.Item
              name={`uniformDeduction-${record.key}`}
              className="m-0 mt-[-4.5px] mb-[-5.5px]"
              rules={[
                {
                  validator: (_, value) => {
            ...
            >
              <Input
                ...
                autoComplete="off"
                autoFocus
                onPressEnter={() => saveDeduction(record)}
                onKeyDown={e => {
                  if (e.key === 'Escape') {
                    setEditingDeductionKey('');
                    form.resetFields([`uniformDeduction-${record.key}`]);
                  }
                }}
              />
            </Form.Item>
        ...
        return (
          <div
            className="group cursor-pointer"
            onClick={() => {
              setEditingDeductionKey(record.key);
              form.setFieldsValue({
                [`uniformDeduction-${record.key}`]: record.uniformDeduction,
              });
            }}
          >
            <span
    ...
    },
    {
      ...
      width: 116,
      render: (_, record: UniformTableRow) => {
        return (
          <span className="text-inactive">
            {(
              Number(record.outstandingBalance ?? 0) +
              Number(record.uniformDisbursement ?? 0) -
              Number(record.uniformDeduction ?? 0)
            ).toLocaleString()}
          </span>
        );
      },
  ...
  const cancel = (record: UniformTableRow) => {
    if (isNewRow(record)) {
  ...
  // Save for uniformDisbursement cell
  const saveDisbursement = async (record: UniformTableRow) => {
    try {
      ...
      if (index > -1) {
        newData[index].uniformDisbursement =
          values[`uniformDisbursement-${record.key}`];
        setData(newData);
        setEditingDisbursementKey('');
        form.resetFields([`uniformDisbursement-${record.key}`]);
      }
    } catch (err) {
      console.error('Save Disbursement failed:', err);
    }
  };

  // Save for uniformDeduction cell
  const saveDeduction = async (record: UniformTableRow) => {
    try {
      const values = await form.validateFields([
        `uniformDeduction-${record.key}`,
      ]);
      const newData = [...data];
      const index = newData.findIndex(item => item.key === record.key);
      if (index > -1) {
        newData[index].uniformDeduction =
          values[`uniformDeduction-${record.key}`];
        setData(newData);
        setEditingDeductionKey('');
        form.resetFields([`uniformDeduction-${record.key}`]);
      }
  ...
  const saveNewRow = async (record: UniformTableRow) => {
    try {
      const values = await form.validateFields([
        'employeeId',
        'uniformDisbursement',
      ...
      if (index > -1) {
        newData[index] = {
          ...
          uniformDisbursement: values.uniformDisbursement,
          uniformDeduction: values.uniformDeduction,
        };
        setData(newData);
        setEditingKey('');
        form.resetFields([
  ...
  const onFinish = async () => {
    try {
      ...
      setSuccessMessage('บันทึกข้อมูลการหักเงินชุดฟอร์มสำเร็จ');
    } catch (error) {
  ...
  const createNewRow = () => {
    const newKey = uuid();
    const newRow: UniformTableRow = {
      key: newKey,
      employeeId: '',
      employeeName: '',
      outstandingBalance: 0,
      uniformDisbursement: 0,
      uniformDeduction: 0,
