...
import { EtcTableType, etcTable } from './MockData';
import './style.css';

type EtcTableRow = EtcTableType & {
  key: string;
};

const EtcTable: React.FC = () => {
  const [form] = Form.useForm();
  const [data, setData] = useState<EtcTableRow[]>(
    etcTable.map(item => ({ ...item, key: item.employeeId }))
  );
  ...
  const isEditing = (record: EtcTableRow) => record.key === editingKey;

  const isNewRow = (record: EtcTableRow) =>
    isEditing(record) && !record.employeeId;

  const getDisbursementColumns = (
    cancel: (record: EtcTableRow) => void
  ): TableColumnsType<EtcTableRow> => [
    {
      title: 'ลำดับ',
      dataIndex: 'index',
      width: 60,
      render: (_, _record: EtcTableRow, index: number) => (
        <span>{index + 1}</span>
      ),
    },
    {
      ...
      width: 88,
      sorter: (a: EtcTableRow, b: EtcTableRow) => {
      ...
      render: (text: string, record: EtcTableRow) => {
        if (isNewRow(record)) {
          return {
            children: (
              <Form.Item
    ...
    },
    {
      ...
      dataIndex: 'deduction',
      width: 116,
      render: (value: number, record: EtcTableRow) => {
        if (isNewRow(record)) {
          return (
            <Form.Item
              name="deduction"
              ...
              <Input
                ...
                onKeyDown={e => {
                  if (e.key === 'Escape') {
                    ...
                    form.resetFields([`deduction-${record.key}`]);
                  }
                }}
              />
            </Form.Item>
        ...
        return (
          <div
            ...
            onClick={() => {
              ...
              form.setFieldsValue({
                [`deduction-${record.key}`]: record.deduction,
              });
            }}
          >
            <span
    ...
    },
    {
      title: 'หมายเหตุ',
      dataIndex: 'note',
      render: (value: string, record: EtcTableRow) => {
        if (isNewRow(record)) {
          return (
            <Form.Item
              name="note"
  ...
  const cancel = (record: EtcTableRow) => {
    if (isNewRow(record)) {
  ...
  const saveDeduction = async (record: EtcTableRow) => {
    try {
      const values = await form.validateFields([`deduction-${record.key}`]);
      const newData = [...data];
      const index = newData.findIndex(item => item.key === record.key);
      if (index > -1) {
        newData[index].deduction = values[`deduction-${record.key}`];
        setData(newData);
        setEditingDeductionKey('');
        form.resetFields([`deduction-${record.key}`]);
      }
  ...
  const saveNote = async (record: EtcTableRow) => {
    try {
  ...
  const saveNewRow = async (record: EtcTableRow) => {
    try {
      ...
      if (index > -1) {
        ...
        setData(newData);
        setEditingKey('');
        form.resetFields(['employeeId', 'deduction', 'note']);
      }
    } catch (err) {
  ...
  const onFinish = async () => {
    try {
      ...
      setSuccessMessage('บันทึกข้อมูลการหักเงินอื่นๆสำเร็จ');
    } catch (error) {
