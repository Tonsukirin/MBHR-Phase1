...
export interface UpdateEmployeeRequestApi
  extends Omit<UpdateEmployeeRequest, 'employee'> {
  employee?: Omit<
    NonNullable<UpdateEmployeeRequest['employee']>,
    'birthDate' | 'startDate'
  > & {
    birthDate?: Dayjs;
    startDate?: Dayjs;
...
const EditEmployeeInfo = ({ params }: { params: { id: number } }) => {
  ...
  const [employeeData, setEmployeeData] =
    useState<UpdateEmployeeRequestApi | null>(null);
  const [employeeOriginalData, setEmployeeOriginalData] =
    useState<UpdateEmployeeRequestApi | null>(null);
  const [employeeShownId, setEmployeeShownId] = useState('');
  ...
  const transformResponseToUpdatePayload = (
    ...
    setEmployeeShownId(shownEmployeeId);

    const {
      updatedAt: _additionalUpdatedAt,
      createdAt: _additionalCreatedAt,
      id: _employeeId,
      employeeId: _employeeShownId,
      ...additionalInfoRest
    } = additionalInfo;
    ...
    return {
      employee: {
        ...employeeData,
        birthDate: dayjs(response.birthDate),
        startDate: dayjs(response.startDate),
      },
      addresses: {
        current: transformAddress(currentAddress),
        permanent: transformAddress(permanentAddress),
      },
      additionalInfo: {
        ...additionalInfoRest,
      },
  ...
  const transformAddress = (address: AddressResponse): Partial<AddressBase> => {
    const {
      id: _id,
      updatedAt: _updatedAt,
      createdAt: _createdAt,
  ...
  useEffect(() => {
    const fetchEmployeeData = async () => {
      ...
      try {
        ...
        const transformedData = transformResponseToUpdatePayload(response.data);
        setEmployeeData(transformedData);
        setEmployeeOriginalData(transformedData);
        form.setFieldsValue(transformedData);

        handleBirthDateChange(form.getFieldValue(['employee', 'birthDate']));
        setIsSameAddress(response.data.isSameAddress);
        setInsuranceEnabled(response.data.isSocialSecurityEnabled);
        if (response.data.additionalInfo.maritalStatus === 'single')
  ...
  const onFinish = async (mainFormValues: CreateEmployeeFormType) => {
    try {
      ...
      if (changedData.employee) {
        if (changedData.employee.birthDate !== undefined) {
          changedData.employee.birthDate =
            transformedForm.employee.birthDate.toISOString();
        }
        if (changedData.employee.startDate !== undefined) {
          changedData.employee.startDate =
            transformedForm.employee.startDate.toISOString();
        }
        if (changedData.employee.isThai !== undefined) {
          changedData.employee.isThai = parseIsThai(
            transformedForm.employee.isThai
          );
        }
      }

      const finalPayload = { data: changedData };

      console.log('Changed fields to submit:', finalPayload);
      const response = await updateEmployee(finalPayload, id);
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-6 h-full">
          ...
          <div className="flex gap-2">
            <ArrowLeftOutlined
              onClick={() =>
                router.push(`/dashboard/employee-management/${id}`)
              }
            />
            <span className="font-semibold text-20">
              {employeeData?.employee ? employeeData?.employee.name : ''}{' '}
              <span className="text-[#6B6B6B] font-regular">
                ({employeeShownId})
              </span>
            </span>
          </div>
                ...
                >
                          ...
                          children: (
                            <div>
                              <div className="flex gap-6">
                                <Form.Item
                                  label="คำนำหน้า"
                                  name={['additionalInfo', 'spouseTitle']}
                                  className="w-[84px]"
                                >
                                  <Select
                                    placeholder="คำนำ"
                                ...
                                <Form.Item
                                  label="ชื่อจริง"
                                  name={['additionalInfo', 'spouseName']}
                                  className="w-[120px]"
                                >
                                  <Input
                                    placeholder="มาเรียม"
                                ...
                                <Form.Item
                                  label="นามสกุล"
                                  name={['additionalInfo', 'spouseSurname']}
                                  className="w-[180px]"
                                >
                                  <Input
                              ...
                              <div className="flex gap-6">
                                <Form.Item
                                  label="อาชีพคู่สมรส"
                                  name={['additionalInfo', 'spouseOccupation']}
                                  className="w-[180px]"
                                >
                                  <Input
                                ...
                                <Form.Item
                                  ...
                                  name={['additionalInfo', 'spousePhone']}
                                  className="w-[160px]"
                                >
                                  <Input
                                ...
                                <Form.Item
                                  label="จำนวนบุตร"
                                  name={['additionalInfo', 'childrenCount']}
                                  className="w-[80px]"
                                >
                                  <InputNumber
                                    placeholder="2"
                  ...
                  <div className="flex flex-col">
                    <div className="flex gap-6">
                      <Form.Item
                        ...
                        name={['additionalInfo', 'educationHighest']}
                      ...
                      <Form.Item
                        label="สาขา"
                        name={['additionalInfo', 'educationMajor']}
                      ...
                      <Form.Item
                        label="คะแนน"
                        name={['additionalInfo', 'educationGpax']}
                    ...
                    <div>
                      <Form.Item
                        label="ชื่อสถานศึกษา"
                        name={['additionalInfo', 'educationName']}
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex gap-3">
                      <Form.Item
                        label="โรงพยาบาล"
                        name={['additionalInfo', 'socialSecurityHospitalName']}
                        className="w-[400px]"
                      >
                        <Input
                      ...
                      <Form.Item
                        ...
                        name={['additionalInfo', 'socialSecurityId']}
                  ...
                  <div className="flex">
                    <div className="flex gap-6">
                      <Form.Item
                        label="ชื่อ - นามสกุล"
                        name={['additionalInfo', 'emergencyName']}
                      ...
                      <Form.Item
                        ...
                        name={['additionalInfo', 'emergencyRelation']}
                      ...
                      <Form.Item
                        label="เบอร์โทรศัพท์"
                        name={['additionalInfo', 'emergencyPhone']}
